...
```{r}
Crm_prs <- as.numeric(as.character(guerry_sp$Crm_prs))
...
Why are we using `as.numeric(as.character(guerry_sp$Crm_prp))` here to get values from data.frame?
```

As we can see from the output of `readOGR(guerry_path)` that: 
```
_"Integer64 fields read as strings:  dept Crm_prs Crm_prp Litercy Donatns Infants Suicids MainCty_"
```
, that means `guerry_sp$Crm_prp` is with data type `factor`, so `as.numeric(as.character())` is used to convert it to a column with numeric values without lossing precisions.
